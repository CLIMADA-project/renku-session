...
cp $WD/renku-session/climada.config $WD/
...
unzip -u *-zenodo.*/*.zip -d $WD/climada/data

echo all set up. you\'re ready to go
echo try: \`python -m impact_forecasting_warning.pipelines.wind_impact_forecast --n-days 1\`
